...
class mesh:
    ...
    def convert_vertices_to_mesh(self, sm_radius=2.0, msh_file='mesh.msh', rmax=1.0):
        ...
        if mda_version[0] == 0 and mda_version[1] >= 19:
            self.neighbor_search_old(cutoff=sm_radius, coords=temp,
                                     box=box)  # neighbor grid search to identify vertex pairs within r < cutoff apart
        elif mda_version[0] == 1 and (mda_version[2] >= 2 or mda_version[1] >= 1):
            self.neighbor_search_new(cutoff=sm_radius, coords=temp,
                                     box=box)  # neighbor grid search to identify vertex pairs within r < cutoff apart
        else:
