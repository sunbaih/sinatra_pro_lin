...
def convert_traj_pdb_aligned(protA, protB, struct_file_A, traj_file_A, struct_file_B, traj_file_B, align_frame=0,
    ...
    for prot, seqsel, struct_file, traj_file in zip([protA, protB], [seqselA, seqselB], [struct_file_A, struct_file_B],
                                                    [traj_file_A, traj_file_B]):
...
def convert_pdb_mesh(n_sample, by_frame =True, protA="protA", protB="protB", sm_radius=4.0, directory_pdb_A=None,
    ...
    if directory_pdb_A == None or directory_pdb_B == None:
        for prot in [protA, protB]:
            if parallel:
                r_prots = Parallel(n_jobs=n_core)(
                    delayed(calc_radius_pdb)(selection='protein', directory=directory, prot=prot, i_sample=i_sample) for
                    i_sample in range(n_sample))
        ...
        for directory_pdb in [directory_pdb_A, directory_pdb_B]:
            if parallel:
                r_prots = Parallel(n_jobs=n_core)(
                    delayed(calc_radius_pdb)(selection='protein', directory_pdb=directory_pdb, filename=filename) for
                    filename in os.listdir(directory_pdb))
                r = np.append(r, r_prots)
            else:
                r_prots = []
